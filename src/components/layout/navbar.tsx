...
export function Navbar() {
  ...
  return (
    <nav className="w-full bg-white border-b border-neutral-200 relative z-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-20 items-center">
          ...
          <div className="hidden md:flex items-center gap-8">
            ...
            <Link href="/posts" className="text-sm font-semibold uppercase tracking-wider bg-neutral-950 text-white px-5 py-2.5 hover:bg-neutral-800 transition-colors">
              Journal
            </Link>
          </div>
      ...
      {/* Mobile Nav */}
      {isOpen && (
        <div className="md:hidden absolute top-20 left-0 w-full bg-white border-b border-neutral-200 py-4 px-4 shadow-xl">
          <div className="flex flex-col gap-4">
            {links.map((link) => (
              <Link 
                key={link.href} 
                href={link.href} 
                className="text-sm font-semibold uppercase tracking-wider text-neutral-500 hover:text-neutral-950 transition-colors py-2"
